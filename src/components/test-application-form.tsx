...
export function TestApplicationForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/apply", {
        ...
        body: JSON.stringify({ name: data.get("name"), email: data.get("email"), provider: "openai", pain: data.get("pain"), source }),
      });
  ...
  return (
    <form className="form-card" onSubmit={handleSubmit}>
      ...
      <label><span>Provider you pay directly</span><select defaultValue="openai" disabled><option value="openai">OpenAI — supported in this test</option></select></label>
      ...
      <small>No API key requested. I&apos;ll contact you personally.</small>
    </form>
  );
}
