...
export async function POST(request: Request) {
  ...
  const provider = typeof body.provider === "string" && providers.has(body.provider) ? body.provider : "";
  if (!name || !email || !email.includes("@") || !provider) return Response.json({ error: "Name, valid email, and provider are required" }, { status: 400 });
