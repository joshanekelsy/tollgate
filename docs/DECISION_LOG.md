...
| 2026-09-05 | Bind retries to a private payload fingerprint and bound proxy inputs and responses. | Idempotency must distinguish changed requests without retaining their content, and public proxy traffic must not create unbounded customer metadata or response memory use. |
| 2026-09-05 | Require exact-origin checks for dashboard mutations. | Project-scoped signed cookies remain the authorization source, while an origin check adds direct protection against cross-site mutation requests. |
