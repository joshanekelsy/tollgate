...
export const resolve = query({
  args: { ...serviceAuthArgs, projectId: v.string(), taskId: v.string() },
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    requireServiceToken(args);
    return ctx.db.query("taskPolicies").withIndex("by_project", (q) => q.eq("projectId", args.projectId)).filter((q) => q.eq(q.field("active"), true)).collect();
  },
});

export const approve = mutation({
  args: { ...serviceAuthArgs, projectId: v.string(), taskId: v.string(), provider: v.optional(provider), approvedModel: v.string(), now: v.number() },
  handler: async (ctx, args) => {
    requireServiceToken(args);
...
export const remove = mutation({
  args: { ...serviceAuthArgs, projectId: v.string(), taskId: v.string(), now: v.number() },
  handler: async (ctx, args) => {
    requireServiceToken(args);
    const existing = await ctx.db.query("taskPolicies").withIndex("by_project_task", (q) => q.eq("projectId", args.projectId).eq("taskId", args.taskId)).unique();
